...
Each existing customer using Giant Swarm Vintage product has been given a presentation about CAPA benefits. We have gathered the most crucial high-level advantages in the list below to refresh your memory:
...
- Cluster apps moved to org namespace - This change will allow to simplify RBAC and enable Gitops managed clusters to be created with pre-defined set of application.
...
- `GP2` volumes not supported - Major of customers are already using `gp3` volumes, as we are refreshing the infrastructure, deprecated `kubernetes.io/aws-ebs` provisioner creating `gp2` volumes will also be removed
