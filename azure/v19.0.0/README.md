...
- [k8s-dns-node-cache](https://github.com/giantswarm/k8s-dns-node-cache-app) - improving cluster DNS performance by running a dns caching agent on cluster nodes as a DaemonSet. If you are using `egress` rules for your pods and you needed to allow traffic towards coreDNS than you might need to change the network policy to make `k8s-dns-node-cache` work properly. If in doubt get in touch with your account engineer before you begin the cluster upgrade process.
