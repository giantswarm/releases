## :zap: Giant Swarm Release 9.1.0 for AWS is now active for you! :zap:

Note: This is a legacy port of release 11.0.0 with Kubernetes 1.16.3 for non-node pool clusters (i.e. cluster versions < 10).
...
- Updated to upstream `Cluster Autoscaler` v1.16.2 - [changelog](https://github.com/kubernetes/autoscaler/releases/tag/cluster-autoscaler-1.16.2).
- Removed CPU limits.
- Migrated to be deployed via an app CR not a chartconfig CR.
...
- Added support for AWS SDK configuration with explicit credentials.
- Removed CPU limits.
