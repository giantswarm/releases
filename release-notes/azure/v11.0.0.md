...
This is the first Giant Swarm release which includes Kubernetes v1.16. As part of this release, we have also removed CPU limits from several supporting components and adjusted priority classes to ensure system reliability under heavy load. Additional details about changes to individual components can be found below.
...
- As previously communicated, Kubernetes v1.16 has removed several API groups which were already deprecated. Please ensure that all affected components have been updated to the 1.16 compatible versions before upgrading clusters from v1.15.
- Previous Giant Swarm releases included Helm and Tiller v2.14 which is not compatible with Kubernetes v1.16. The Tiller used by Giant Swarm operators in the kube-system namespace has therefore been updated to the compatible v2.16. Any Tiller instances deployed in other namespaces will likewise need to be updated to v2.16 or greater to continue to function after an upgrade.
